...
jQuery.TreeTable = function(tree_id, heads, tNodes) {
    if ($("#" + tree_id).length > 0) {
        ...
        for (var i = 0; i < column_count; i++) {
            head_html += "<td class='alignCenter'>" + heads[i] + "</td>";
        }
        head_html += "</tr></thead>";
        ...
        for (var i = 0, l = newNodes.length; i < l; i++) {
            var node = newNodes[i];
            var tdOne = "<td>" + (node.td[0] ? node.td[0] : "") + "</td>";
            body_html += "<tr data-tt-id='" + node.id + "' " + (node.pId ? "data-tt-parent-id='" + node.pId + "'" : "") + "><td class = 'td_left'><span class='" + (node.isParent ? "parent" : "children") + "'>" + node.name + "</span></td>";
