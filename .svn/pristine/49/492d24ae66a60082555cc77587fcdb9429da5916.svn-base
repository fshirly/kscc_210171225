define(["base","datatables.net","commonUtil","app/commonApp"],function(base,DataTable,commonUtil,common){
	...
	function setScroll(){
		...
		var height = parseInt($(".middleContent").height());//* 0.5;
        $("#carouselItem").height(height);
		$(".carousel-inner").height(height);//中间内容的高度
		var innerHeight = $(".carousel-inner").height();
		$(".uploadPictures").height((innerHeight-5)/2);
		$(".uploadPictures .uploadDiv").css({"padding-top":((innerHeight-5)/2-202)/2,"padding-bottom":((innerHeight-5)/2-202)/2});//197
