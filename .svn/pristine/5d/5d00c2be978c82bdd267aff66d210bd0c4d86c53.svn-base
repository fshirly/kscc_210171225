define(["base","datatables.net","app/commonApp"],function(base,DataTable,common){
    ...
    function setTableHospital(){
        grid = $("#tblHospital").DataTable({
            ...
            "columns":[
                {"title":"序号","sWidth":"5%"},
                {"title":"医院名称", "data":"hospitalName","sWidth":"22%"},
                /*{"title":"医院简介", "data":"hospitalContent","sWidth":"15%"},*/
                {"title":"网站链接", "data":"hospitalUrl","sWidth":"25%"},
                {"title":"医院地址", "data":"location","sWidth":"25%"},
                {"title":"操作", "data":"id","sWidth":"23%"}
            ],
            "columnDefs":[
                {
                ...
                $(".hospitalEdit").off().on("click",function(){
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            ...
                            $("#detailAdress").val(data.detailAddress);
                            setAreaEdit(data.location);
                            $('#editHosModal').on('shown.bs.modal', function () {
                    	...
                    	base.form.validate({
                            ...
                            passCallback:function(){
                            	var params={
                                        ...
                                        "detailAddress":$("#detailAdress").val()
                                    }
    ...
    function setAddHosNext(){
    	$(".hosNext").off().on("click",function(){
	    	locationValAdd = $("#locationAdd").val();
	       	if(locationValAdd==""||locationValAdd=="请选择省/请选择市/请选择县"){
	       		$("#tipMessage").show();
