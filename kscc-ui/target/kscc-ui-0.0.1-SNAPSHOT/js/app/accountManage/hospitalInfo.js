define(["base","datatables.net","app/commonApp"],function(base,DataTable,common){
    ...
    function editHos(){
    	$(".editHos").off().on("click",function(){
    		...
    		$("#hospitalInfoForm textarea").attr("placeHolder","不能超过300个汉字");
			...
			// 	</div>
    		var areaStr="<label class='text-right belongCity'style='height:50px;line-height:50px'>医院地址：</label>"+
    			        "<div class='areaSelect areaSelectInfo input-width' style='margin-top:11px;' role='{required:true}'></div><dfn style='height:50px;line-height:50px;'>*</dfn>"+
                        "<div style='color:red;padding-left:120px;font-size:14px;display:none;' id='tipMessage'>必填项</div>";
    		$("#hosLocation").append(areaStr);
    		setAreaInfo(locationInfo);//医院地址
    		setImageUpload();//上传医院logo
    		//保存
    		$(".saveHosEdit").off().on("click",function(){
				var locationValEdit = $(".pick-province").text()+"/"+$(".pick-city").text()+"/"+$(".pick-county").text();
    			//var locationValEdit = $("#locationInfo").val();
    	       	if(locationValEdit==""||locationValEdit=="请选择省/请选择市/请选择县"){
    	       		$("#tipMessage").show();
    		...
    		$(".cancelHosEdit").off().on("click",function(){
    			...
    			$(".setBtns").html("<button class='btn btn-primary editHos'>编辑</button>");
    			$("#hosLocation").empty();
    			var areaStr="<label class='text-right belongCity'>医院地址：</label>"+
    ...
    function setAreaInfo(location){
		location==undefined?"province/city/county":location;
    	...
    	base.form.areaSelect({
            ...
            id:"locationInfo",
            option:{
              "format":location//"province/city/county"
            }
         });
